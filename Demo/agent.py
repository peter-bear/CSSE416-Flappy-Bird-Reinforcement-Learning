...
class DuelingAgent(Agent):
    def __init__(self, env, arg):
        super().__init__(env, arg)  # Initialize base class attributes
        # Replace the Q-network and target Q-network with dueling networks
        self.Net = DuelingDQN(arg.Frames, arg.action_dim).to(self.arg.cuda)
        self.targetNet = DuelingDQN(arg.Frames, arg.action_dim).to(self.arg.cuda)
